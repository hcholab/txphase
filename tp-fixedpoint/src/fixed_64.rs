...
impl<const F: usize> TpFixed64<F> {
    ...
    pub fn mul_small_const(self, other: Self) -> Self {
        let other = other.inner.expose();
        debug_assert_eq!(other >> F, 0);
        for _i in 0..F {}
        todo!()
    }
        ...
        Zip::from(a_128.rows())
            ...
            .for_each(|a_row, mut c_row| {
                Zip::from(b_128.columns())
                    .and(&mut c_row)
                    .for_each(|b_col, c_e| {
                        *c_e = new_self!((TpI128::dot(
                            a_row.as_slice().unwrap(),
                            b_col.as_slice().unwrap()
                        ) >> F as u32)
                            .into())
                    })
            });
...
impl<const F: usize, S, S2> Dot<ArrayBase<S2, Ix1>> for ArrayBase<S, Ix1>
where
    S: Data<Elem = TpFixed64<F>>,
    S2: Data<Elem = TpFixed64<F>>,
{
    type Output = TpFixed64<F>;
    fn dot(&self, rhs: &ArrayBase<S2, Ix1>) -> TpFixed64<F> {
        TpFixed64::<F>::dot(self.view(), rhs.view())
    }
}

impl<const F: usize, S, S2> Dot<ArrayBase<S2, Ix1>> for ArrayBase<S, Ix2>
where
    S: Data<Elem = TpFixed64<F>>,
    S2: Data<Elem = TpFixed64<F>>,
{
    type Output = Array1<TpFixed64<F>>;
    fn dot(&self, rhs: &ArrayBase<S2, Ix1>) -> Array1<TpFixed64<F>> {
        Zip::from(self.rows()).map_collect(|r| TpFixed64::<F>::dot(r, rhs.view()))
    }
}

impl<const F: usize, S, S2> Dot<ArrayBase<S2, Ix2>> for ArrayBase<S, Ix1>
where
    S: Data<Elem = TpFixed64<F>>,
    S2: Data<Elem = TpFixed64<F>>,
{
    type Output = Array1<TpFixed64<F>>;
    fn dot(&self, rhs: &ArrayBase<S2, Ix2>) -> Array1<TpFixed64<F>> {
        Zip::from(rhs.columns()).map_collect(|c| TpFixed64::<F>::dot(c, self.view()))
    }
}

impl<const F: usize, S, S2> Dot<ArrayBase<S2, Ix2>> for ArrayBase<S, Ix2>
where
    S: Data<Elem = TpFixed64<F>>,
    S2: Data<Elem = TpFixed64<F>>,
{
    type Output = Array2<TpFixed64<F>>;
    fn dot(&self, rhs: &ArrayBase<S2, Ix2>) -> Array2<TpFixed64<F>> {
        let mut c =
            Array2::<TpFixed64<F>>::from_elem((self.nrows(), rhs.ncols()), TpFixed64::<F>::ZERO);
        TpFixed64::<F>::matmul(self.view(), rhs.view(), c.view_mut());
        c
    }
}
