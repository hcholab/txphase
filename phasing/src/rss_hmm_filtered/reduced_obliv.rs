...
use crate::inner::*;
use crate::rss_hmm_filtered::filtered_block::FilteredBlockSliceObliv;
use crate::tp_value;
use ndarray::{
...
thread_local! {
    ...
    pub static COMB11: RefCell<Duration> = RefCell::new(Duration::ZERO);
    pub static COMB12: RefCell<Duration> = RefCell::new(Duration::ZERO);
    pub static COMB2: RefCell<Duration> = RefCell::new(Duration::ZERO);
    pub static COMB21: RefCell<Duration> = RefCell::new(Duration::ZERO);
    pub static COMB22: RefCell<Duration> = RefCell::new(Duration::ZERO);
    pub static COMB23: RefCell<Duration> = RefCell::new(Duration::ZERO);
...
impl HmmReduced {
    pub fn fwbw<'a, 'b>(
        ...
        blocks: &[FilteredBlockSliceObliv<'a, 'b>],
    ...
    fn init_c_prob<'a, 'b>(
        is_forward: bool,
        block: &FilteredBlockSliceObliv<'a, 'b>,
        graph_pos: G,
        eprob: Real,
    ...
    fn block_transition_backward<'a, 'b>(
        ...
        prev_block: &FilteredBlockSliceObliv<'a, 'b>,
        cur_block: &FilteredBlockSliceObliv<'a, 'b>,
        cur_graph_pos: G,
        do_collapse: Bool,
        #[cfg(not(feature = "obliv"))] eprob: Real,
        rprob: Real,
    ...
    fn combine_block<'a, 'b>(
...

        {
            ...
            Zip::from(forward.alpha_pre.rows())
                .and(forward_alpha_post)
                .and(backward.alpha_pre.rows())
                .and(backward_alpha_post)
                .and(&block.index_map)
                .and(&block.full_filter)
                .for_each(|f_pre, &f_pos, b_pre, &b_pos, &i, &filter| {
                    to_filter.push((
                        filter.select(U16::protect(i), U16::protect(u16::MAX)),
                        f_pre.to_owned(),
                        f_pos,
                        b_pre.to_owned(),
                        b_pos,
            ...
            #[inline(always)]
            fn lt_eq(
                a: &(U16, Array1<Real>, Real, Array1<Real>, Real),
                b: &(U16, Array1<Real>, Real, Array1<Real>, Real),
            ) -> TpBool {
                a.0.tp_lt_eq(&b.0)
            }
            ...
            fn cond_swap_fn(
                cond: TpBool,
                a: &mut (U16, Array1<Real>, Real, Array1<Real>, Real),
                b: &mut (U16, Array1<Real>, Real, Array1<Real>, Real),
            ) {
                cond.cond_swap(&mut a.0, &mut b.0);
                Zip::from(&mut a.1)
                    .and(&mut b.1)
                    .for_each(|a, b| cond.cond_swap(a, b));
                cond.cond_swap(&mut a.2, &mut b.2);
                Zip::from(&mut a.3)
                    .and(&mut b.3)
                    .for_each(|a, b| cond.cond_swap(a, b));
                cond.cond_swap(&mut a.4, &mut b.4);
            }

            obliv_utils::bitonic_sort::bitonic_sort_by(&mut to_filter, true, gt_eq, cond_swap_fn);

            let mut f_pre_arr = Array2::<Real>::zeros((P, max_k));
            let mut f_post_arr = Array1::<Real>::zeros(max_k);
            let mut b_pre_arr = Array2::<Real>::zeros((P, max_k));
            let mut b_post_arr = Array1::<Real>::zeros(max_k);

            let mut hap_ids = Array1::<U16>::from_elem(max_k, U16::protect(0));

            to_filter.into_iter().take(max_k).enumerate().for_each(
                |(i, (hap_id, f_pre, f_post, b_pre, b_post))| {
                    hap_ids[i] = hap_id;
                    f_pre_arr.column_mut(i).assign(&f_pre);
                    f_post_arr[i] = f_post;
                    b_pre_arr.column_mut(i).assign(&b_pre);
                    b_post_arr[i] = b_post;
                },
            );

            let mut hap_ids_filter = Array1::<U16>::from_elem(max_k, U16::protect(u16::MAX));
            for (window, u) in hap_ids
                .as_slice()
                .unwrap()
                .windows(2)
                .zip(hap_ids_filter.iter_mut())
            {
                *u = window[0]
                    .tp_not_eq(&window[1])
                    .select(window[0], U16::protect(u16::MAX));
            }
            *hap_ids_filter.last_mut().unwrap() = *hap_ids.last().unwrap();

            //for (i, (_, probs)) in to_filter.into_iter().take(n_unique_haps).enumerate() {
            //alpha_r.column_mut(i).assign(&probs);
            //}
            //});
            ...
            use crate::rss_hmm_filtered::ab_elem::AbElem;
            let mut to_filter = vec![unsafe { AbElem::uninit() }; max_k];

            for (&id, e) in hap_ids_filter.iter().zip(to_filter.iter_mut()) {
                e.id = id;
            }

            // 11
            Zip::indexed(f_pre_arr.rows()).for_each(|i, f_r| {
                let mut prod = b_pre_arr.to_owned();
                Zip::from(prod.rows_mut()).for_each(|mut p_r| {
                    p_r *= &f_r;
                });

                let mut cur_sum = TpU64x8::ZERO;
                Zip::indexed(prod.columns())
                    .and(&hap_ids_filter)
                    .for_each(|j, prod_r, &id| {
                        let mut prod_r_ = TpU64x8::ZERO;
                        for (&src, tar) in prod_r.iter().zip(prod_r_.as_mut_array().iter_mut()) {
                            *tar = unsafe { std::mem::transmute(src) };
                        }
                        cur_sum += prod_r_;

                        to_filter[j].e11[i] = cur_sum;
                        let cond = id.tp_not_eq(&U16::protect(u16::MAX));
                        cur_sum = cond.select(TpU64x8::ZERO, cur_sum);
                    });
            });

            // 10
            {
                let mut prod = f_pre_arr.to_owned();
                Zip::from(prod.rows_mut()).for_each(|mut p_r| {
                    p_r *= &b_post_arr;
                });
            ...
            // 00
            {
                let prod = &f_post_arr * &b_post_arr;
                let mut cur_sum = Real::ZERO;
                Zip::indexed(&prod)
                    .and(&hap_ids_filter)
                    .for_each(|j, &e, &id| {
                        cur_sum += e;
                        to_filter[j].e00 = cur_sum;
                        let cond = id.tp_not_eq(&U16::protect(u16::MAX));
                        cur_sum = cond.select(Real::ZERO, cur_sum);
                    });
            }

            Zip::indexed(&block.filter).for_each(|i, b| {
                let id = b.select(U16::protect(u16::MAX), U16::protect(i as u16));
                let mut new_elem = unsafe { AbElem::uninit() };
                new_elem.id = id;
                to_filter.push(new_elem);
            });
            ...
            obliv_utils::bitonic_sort::bitonic_sort_top_s(&mut to_filter, n_unique_haps, true);
            ...
            //Zip::from(f_pre_arr.rows())
            //.and(alpha_11.outer_iter_mut())
            //.for_each(|f_r, mut alpha_r| {
            //#[cfg(feature = "benchmarking")]
            //let t = Instant::now();
            //let mut prod = b_pre_arr.to_owned();
            //Zip::from(prod.rows_mut()).for_each(|mut p_r| {
            ...
            //let mut cur_sum = TpU64x8::protect(u64x8::splat(0));

            //let mut to_filter = Vec::with_capacity(max_k + n_unique_haps);

            //Zip::from(prod.columns())
            //.and(&hap_ids_filter)
            //.for_each(|prod_r, &id| {
            //let mut prod_r_ = u64x8::splat(0);
            //for (&src, tar) in prod_r.iter().zip(prod_r_.as_mut_array().iter_mut())
            //{
            //*tar = unsafe { std::mem::transmute(src) };
            //}
            //cur_sum += TpU64x8::protect(prod_r_);
            //to_filter.push((id, cur_sum.clone()));
            //let cond = id.tp_not_eq(&U16::protect(u16::MAX));
            //cur_sum = cond.select(TpU64x8::protect(u64x8::splat(0)), cur_sum);
            //});

            //Zip::indexed(&block.filter).for_each(|i, b| {
            //let id = b.select(U16::protect(u16::MAX), U16::protect(i as u16));
            //to_filter.push((id, TpU64x8::protect(u64x8::splat(0))));
            //});
            ...
            //obliv_utils::bitonic_sort::bitonic_sort_by(
            //&mut to_filter,
            //true,
            //gt_eq_3,
            //cond_swap_fn_3,
            ...
            //for (i, (_, probs)) in to_filter.into_iter().take(n_unique_haps).enumerate() {
            //let probs = probs.to_array();
            //let probs = unsafe {
            //std::slice::from_raw_parts(
            //&probs as *const _ as *const Real,
            //probs.len(),
            //)
            //};
            //let probs = ArrayView1::from(probs);
            //alpha_r.column_mut(i).assign(&probs);
            //}
            //});
