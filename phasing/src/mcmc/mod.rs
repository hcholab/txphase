...
thread_local! {
    ...
    pub static FILTER_FULL: RefCell<Duration> = RefCell::new(Duration::ZERO);
    pub static PBWT: RefCell<Duration> = RefCell::new(Duration::ZERO);
    pub static PBWT_FULL: RefCell<Duration> = RefCell::new(Duration::ZERO);
...
impl<'a> Mcmc<'a> {
    pub fn run(
        ...
        for iter in iterations_iternal {
            mcmc.iteration_test_filter(iter, &mut bprob_memory, &mut rng, id);
        }
    ...
    fn iteration_test_filter(
        ...
        // expand to reference panel
        let mut full_ref_panel =
            Array2::<i8>::zeros((self.params.ref_panel.n_sites, self.params.ref_panel.n_haps));
        for (src, mut tar) in self
            .params
            .ref_panel
            .iter()
            .zip(full_ref_panel.rows_mut().into_iter())
        {
            tar.assign(&src);
        }
        ...
        let neighbors = {
            ...
            {
                let t = Instant::now();
                let _nn = crate::full_neighbor_finding::find_top_neighbors(
                    &h_0,
                    &h_1,
                    self.params.s,
                    full_ref_panel.rows().into_iter().map(|v| v.to_owned()),
            ...
            windows.into_iter().enumerate()
        {
            ...
            let full_unfolded = {
                let t = Instant::now();
                let full = full_ref_panel.slice(s![start_w..end_w, ..]);

                // pack to U64
                let (unfolded, _filter, _n_full_states) = filter_full(neighbors_w, full.view());
            ...
            assert_eq!(
                unfolded.map(|v| v.expose()),
                full_unfolded.map(|v| v.expose())
            );
        ...
        println!(
            "\tPBWT on Compressed Ref Panel: {:?} ms",
            PBWT.with(|v| *v.borrow()).as_millis()
        );
        println!(
            "\tPBWT on Full Ref Panel: {:?} ms",
            PBWT_FULL.with(|v| *v.borrow()).as_millis()
        );

        println!(
            "\tFilter Compressed Ref Panel: {:?} ms",
            FILTER.with(|v| *v.borrow()).as_millis()
        );
        println!(
            "\tFilter Full Ref Panel: {:?} ms",
            FILTER_FULL.with(|v| *v.borrow()).as_millis()
        );
