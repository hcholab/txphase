...
stack-size=0x200000
heap-size=$HEAP_SIZE
threads=1" > $TAR_DIR/Cargo.toml

$MAIN_DIR/experiments/sgx/build_sgx.sh bin_sgx
$MAIN_DIR/experiments/sgx/run_sgx.sh bin_sgx $MAIN_DIR/profiles/dataset/1kg.sh "$PHASING_OPTIONS" $TAR_DIR $TAR_DIR | tee $TAR_DIR/log.txt
