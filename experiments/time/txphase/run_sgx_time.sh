...
stack-size=0x200000
heap-size=$HEAP_SIZE
threads=1" > $OUTPUT_DIR_SIZE/Cargo.toml

mkdir -p bin
$MAIN_DIR/experiments/sgx/build_sgx.sh bin && \
    $MAIN_DIR/experiments/sgx/run_sgx.sh bin $DATASET "$PHASING_OPTIONS" $OUTPUT_DIR_SIZE $OUTPUT_DIR_SIZE | tee $OUTPUT_DIR_SIZE/log.txt
