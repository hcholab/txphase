...
pub fn genetic_map_from_csv_path(path: &Path) -> anyhow::Result<Vec<(u32, f32)>> {
...
pub fn interpolate_cm(genetic_map: &[(u32, f32)], sites: &[Site]) -> Vec<f32> {
    let mut interpolated = Vec::<f32>::with_capacity(sites.len());

    let first_map = genetic_map.first().unwrap();
    let last_map = genetic_map.last().unwrap();
    let mean_rate = (last_map.1 - first_map.1) / (last_map.0 - first_map.0) as f32;
...
fn interpolate_cm_middle(
    ...
    let rate = (last_map.1 - first_map.1) / (last_map.0 - first_map.0) as f32;
    interpolate_cm_tail(first_map, rate, sites, out);
}

fn interpolate_cm_head(first_map: &(u32, f32), mean_rate: f32, sites: &[Site], out: &mut Vec<f32>) {
    sites.iter().for_each(|s| {
        let dist = first_map.0 - s.pos;
        let cm = first_map.1 - mean_rate * dist as f32;
        out.push(cm);
    });
}

fn interpolate_cm_tail(last_map: &(u32, f32), mean_rate: f32, sites: &[Site], out: &mut Vec<f32>) {
    sites.iter().for_each(|s| {
        let dist = s.pos - last_map.0;
        let cm = last_map.1 + mean_rate * dist as f32;
        out.push(cm);
    });
}
